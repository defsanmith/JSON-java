...
public class XMLTest {
    ...
    public void testWithWhitespaceTrimmingDisabled() {
        ...
        JSONObject actualJson = XML.toJSONObject(originalXml,
                new XMLParserConfiguration().withShouldTrimWhitespace(false));
    ...
    public void shouldTrimWhitespaceDoesNotSupportTagsEqualingCDataTagName() {
        // When using withShouldTrimWhitespace = true, input containing tags with same
        // name as cDataTagName is unsupported and should not be used in conjunction
        String originalXml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
                "<addresses>\n" +
                "   <address>\n" +
        ...
        JSONObject actualJson = XML.toJSONObject(originalXml,
                new XMLParserConfiguration().withShouldTrimWhitespace(false).withcDataTagName("content"));
    ...
    public void shouldTrimWhitespaceEnabledDropsTagsEqualingCDataTagNameButValueRemains() {
        ...
        JSONObject actualJson = XML.toJSONObject(originalXml,
                new XMLParserConfiguration().withShouldTrimWhitespace(true).withcDataTagName("content"));
    ...
    public void testWithWhitespaceTrimmingEnabled() {
        ...
        JSONObject actualJson = XML.toJSONObject(originalXml,
                new XMLParserConfiguration().withShouldTrimWhitespace(true));
        String expectedJsonString = "{\"testXml\":\"Test Whitespace String\"}";
        JSONObject expectedJson = new JSONObject(expectedJsonString);
        Util.compareActualVsExpectedJsonObjects(actualJson, expectedJson);
    }
    ...
    public void clarifyCurrentBehavior() {
        ...
        // After reverting the code, amount is stored as numeric, and phone is stored as
        // string
        String str1 = "    <datatypes>\n" +
        ...
        // After reverting the code, value is still stored as a number. This is due to
        // how XML.isDecimalNotation() works
     ...
     * 
     * This test checks the functionality of extracting a specific JSON object from
     * a simple XML document at a given path.
     */
    @Test
    public void testBasicPathExtraction() throws Exception {
        // Arrange
        String xmlInput = "<root><person><name>John</name><age>30</age></person></root>";
        StringReader xmlReader = new StringReader(xmlInput);
        JSONPointer pathPointer = new JSONPointer("/root/person");

        // Act
        JSONObject extractedJson = XML.toJSONObject(xmlReader, pathPointer);

        // Assert
        assertNotNull("Extracted JSON should not be null", extractedJson);
        assertEquals("Extracted JSON should contain the name", "John", extractedJson.getString("name"));
        assertEquals("Extracted JSON should contain the age", 30, extractedJson.getInt("age"));
    }

    /**
     * Test Case 2: Handle Path with Trailing Slash
     * 
     * This test ensures the method correctly handles paths with a trailing slash,
     * and normalizes it accordingly.
     */
    @Test
    public void testTrailingSlashNormalization() throws Exception {
        // Arrange
        String xmlInput = "<root><data><items><item id=\"1\">First</item><item id=\"2\">Second</item></items></data></root>";
        StringReader xmlReader = new StringReader(xmlInput);
        JSONPointer pathPointer = new JSONPointer("/root/data/items/"); // Trailing slash
        ...
        // Assert
        assertNotNull("Extracted JSON should not be null", extractedJson);
        assertTrue("Extracted JSON should contain item data", extractedJson.has("item"));
    }

    /**
     * Test Case 3: Empty Path (Return Entire Document)
     * 
     * This test checks that an empty path returns the entire XML document as a JSON
     * object.
     */
    @Test
    public void testEmptyPath() throws Exception {
        // Arrange
        String xmlInput = "<root><simple>value</simple></root>";
        StringReader xmlReader = new StringReader(xmlInput);
        JSONPointer pathPointer = new JSONPointer(""); // Empty path

        // Act
        JSONObject fullJson = XML.toJSONObject(xmlReader, pathPointer);

        // Assert
        assertNotNull("Full JSON should not be null", fullJson);
        assertTrue("Root should be present in the JSON", fullJson.has("root"));
        JSONObject root = fullJson.getJSONObject("root");
     ...
     * 
     * This test ensures the method throws a JSONException when the specified path
    ...
    public void testPathNotFound() throws Exception {
        ...
        StringReader xmlReader = new StringReader(xmlInput);
        JSONPointer pathPointer = new JSONPointer("/root/nonexistent");

        // Act - This should throw JSONException
        JSONObject result = XML.toJSONObject(xmlReader, pathPointer);
    }

    /* CHANGES 2 START HERE */

    /**
     * Test Case 1: Replace Entire XML Document with JSON
     * 
     * This test verifies that using an empty path or "/" replaces the entire XML
     * document with a new JSON object.
     */
    @Test
    public void testReplaceEntireDocument() throws Exception {
        ...
        JSONObject newJsonContent = new JSONObject()
                .put("newRoot", new JSONObject().put("newData", "newValue"));

        // Act
        JSONObject updatedJson = XML.toJSONObject(xmlReader, pathPointer, newJsonContent);

        // Assert
        assertNotNull("Updated JSON should not be null", updatedJson);
        assertTrue("Updated JSON should contain 'newRoot'", updatedJson.has("newRoot"));
        JSONObject newRoot = updatedJson.getJSONObject("newRoot");
        assertEquals("New root should contain 'newData'", "newValue", newRoot.getString("newData"));
    }

    /**
     * Test Case 2: Replace Nested Element (Two-Level Path)
     * 
     * This test verifies replacing an element at a two-level deep path within
     * the XML document.
     */
    @Test
    public void testReplaceNestedElement() throws Exception {
        // Arrange
        String xmlInput = "<root><parent><child1>value1</child1><child2>value2</child2></parent></root>";
        StringReader xmlReader = new StringReader(xmlInput);
        JSONPointer pathPointer = new JSONPointer("/root/parent/child1");
        JSONObject replacementJson = new JSONObject().put("updatedValue", "replaced");

        // Act
        JSONObject updatedJson = XML.toJSONObject(xmlReader, pathPointer, replacementJson);

        // Assert
        assertNotNull("Updated JSON should not be null", updatedJson);
        JSONObject parent = updatedJson.getJSONObject("root").getJSONObject("parent");
        assertTrue("Parent should contain 'child1'", parent.has("child1"));
        assertEquals("Child1 should be replaced", replacementJson.toString(), parent.get("child1").toString());
        assertEquals("Child2 should remain unchanged", "value2", parent.getString("child2"));
    }

    /**
     * Test Case 3: Replace Deeply Nested Element
     * 
     * This test verifies replacing a deeply nested element (more than two levels
     * deep).
     */
    @Test
    public void testReplaceDeepNestedElement() throws Exception {
        // Arrange
        String xmlInput = "<root><level1><level2><level3><target>original</target></level3></level2></level1></root>";
        StringReader xmlReader = new StringReader(xmlInput);
        JSONPointer pathPointer = new JSONPointer("/root/level1/level2/level3/target");
        JSONObject replacementJson = new JSONObject().put("newContent", "deep replacement");
        ...
        // Assert
        assertNotNull("Updated JSON should not be null", updatedJson);
        Object target = updatedJson.getJSONObject("root")
