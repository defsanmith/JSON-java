...
public class XML {
     ...
     * booleans but will instead be the exact value as seen in the XML document
     * depending
     * on how flag is set.
     *
     * @param reader              The XML source reader.
     * @param keepNumberAsString  If true, then numeric values will not be coerced
     *                            into
     *                            numeric values and will instead be left as strings
     * @param keepBooleanAsString If true, then boolean values will not be coerced
     *                            into
     *                            * numeric values and will instead be left as
     *                            strings
     * @return A JSONObject containing the structured data from the XML string.
     * @throws JSONException Thrown if there is an errors while parsing the string
     */
    public static JSONObject toJSONObject(Reader reader, boolean keepNumberAsString, boolean keepBooleanAsString)
            throws JSONException {
     ...
     * @throws JSONException Thrown if there is an errors while parsing the string
     */
    public static JSONObject toJSONObject(String string, boolean keepNumberAsString, boolean keepBooleanAsString)
            throws JSONException {
    ...
    /*
     * Refactored version: variable names clarified, comments rewritten for clarity.
     * Public method signatures and overall behavior remain unchanged.
     */

    /**
     * Streams a well‑formed XML document and converts only the subtree identified
     * by
     * {@code path} into a {@link JSONObject}. The parser stops as soon as the
     * pointer is satisfied, minimising unnecessary reads.
     *
     * @param reader XML source.
     * @param path   JSONPointer that selects the desired subtree.
     * @return JSONObject representing the selected subtree.
     * @throws JSONException if parsing fails or the path is absent.
     */
    public static JSONObject toJSONObject(Reader reader, JSONPointer path) throws JSONException {
        // --- normalise the path so it never ends with a slash ------------------
        String normalisedPath = path.toString();
        if (normalisedPath.endsWith("/")) {
            normalisedPath = normalisedPath.substring(0, normalisedPath.length() - 1);
            path = new JSONPointer(normalisedPath);
        }

        // Resolve each path segment upfront for quick checks later on
        String[] segments = splitPath(normalisedPath);
        if (segments.length == 0) {
            // Empty pointer ➜ return the whole document via existing helper
            return toJSONObject(reader);
        }

        // Wrap the incoming Reader so we can keep track of progress (defensive)
        ProgressReader trackingReader = new ProgressReader(reader);
        long previousPosition = 0;

        JSONObject scratch = new JSONObject(); // incremental parse target
        JSONObject extractedObject = null; // final result holder
        boolean subtreeFound = false;

        XMLParserConfiguration cfg = XMLParserConfiguration.ORIGINAL;
        XMLTokener tokenizer = new XMLTokener(trackingReader, cfg);

        while (tokenizer.more() && !subtreeFound) {
            tokenizer.skipPast("<"); // move to next tag start
            if (!tokenizer.more())
                break;

            // Parse one top‑level element into scratch
            parse(tokenizer, scratch, null, cfg, 0);

            // See if the desired subtree has appeared
            try {
                Object maybe = path.queryFrom(scratch);
                if (maybe != null) {
                    subtreeFound = true;

                    if (maybe instanceof JSONObject) {
                        extractedObject = (JSONObject) maybe;
                    } else {
                        extractedObject = new JSONObject();
                        extractedObject.put(segments[segments.length - 1], maybe);
                    }
                }
            } catch (JSONPointerException ignore) {
                // not yet – keep streaming
            }

            long currentPos = trackingReader.getPosition();
            if (currentPos == previousPosition)
                break; // safeguard against stalls
            previousPosition = currentPos;
        }

        if (subtreeFound && extractedObject != null) {
    ...
    /**
     * Reader wrapper that records the number of characters consumed.
     */
    private static class ProgressReader extends Reader {
        private final Reader delegate;
        private long position = 0;

        ProgressReader(Reader delegate) {
    ...
    /** Split the JSONPointer string into unescaped path segments. */
    private static String[] splitPath(String pointer) {
        ...
        String[] raw = pointer.split("/");
        for (int i = 0; i < raw.length; i++) {
            raw[i] = JSONPointer.unescapeToken(raw[i]);
        }
        return raw;
    }

    /**
     * Parses XML and replaces the subtree specified by {@code path} with
     * {@code replacement}. Attempts fast‑path handling for shallow paths.
     */
    public static JSONObject toJSONObject(Reader reader, JSONPointer path, JSONObject replacement)
            throws JSONException {
        // Replacing the entire document? validate XML only then return replacement.
        if (path.toString().isEmpty() || "/".equals(path.toString())) {
            XMLTokener tok = new XMLTokener(reader, XMLParserConfiguration.ORIGINAL);
            if (!tok.more())
                throw new JSONException("Empty XML document");
            return replacement;
        }

        // Trim any trailing slash so path logic is consistent
        String rawPath = path.toString();
        boolean trailingSlash = rawPath.endsWith("/");
        if (trailingSlash) {
            rawPath = rawPath.substring(0, rawPath.length() - 1);
            path = new JSONPointer(rawPath);
        }

        String[] segments = splitPath(rawPath);
        if (segments.length <= 2) {
            return shallowReplace(reader, segments, replacement, trailingSlash);
        }
        ...
        while (tokenizer.more()) {
            ...
            if (!tokenizer.more())
                break;
            parse(tokenizer, root, null, cfg, 0);

            try {
                JSONPointer parentPtr = parentPointer(path);
                Object parent = parentPtr.queryFrom(root);
                if (parent instanceof JSONObject) {
                    ...
                    if (trailingSlash && replacement.length() == 1) {
                        String onlyKey = replacement.keys().next();
                        parentObj.put(leafKey, replacement.get(onlyKey));
                    } else {
                        parentObj.put(leafKey, replacement);
                    }
                    break; // replacement done
                }
            } catch (JSONPointerException ignore) {
                /* continue streaming */ }
        ...
        try {
            path.queryFrom(root); // verify replacement accessible
            return root;
    ...
    /** Fast path for one‑ or two‑segment pointers. */
    private static JSONObject shallowReplace(Reader reader, String[] segments, JSONObject replacement,
            boolean trailingSlash) throws JSONException {
        JSONObject doc = new JSONObject();
        XMLParserConfiguration cfg = XMLParserConfiguration.ORIGINAL;
        XMLTokener tok = new XMLTokener(reader, cfg);
        ...
        if (segments.length == 1) {
            ...
            while (tok.more()) {
                ...
                if (token instanceof String && token.equals(target)) {
                    // Skip original subtree quickly
                    int depth = 1;
                    while (depth > 0 && tok.more()) {
                        token = tok.nextToken();
                        if (token == LT) {
                            if (tok.nextToken() == SLASH)
                                depth--;
                            else
                                depth++;
                        }
                    ...
                    if (trailingSlash && replacement.length() == 1) {
                        String onlyKey = replacement.keys().next();
                        doc.put(target, replacement.get(onlyKey));
                    } else
                        doc.put(target, replacement);
                } else {
                    tok.back();
                    parse(tok, doc, null, cfg, 0);
                }
            }
        } else { // two‑segment path
            ...
            while (tok.more()) {
                ...
                if (!tok.more())
                    break;
                Object token = tok.nextToken();
                if (token instanceof String && token.equals(parentKey)) {
                    JSONObject parentObj = new JSONObject();
                    boolean childReplaced = false;

                    // skip to '>' of parent start tag
                    while (tok.more() && token != GT)
                        token = tok.nextToken();

                    while (tok.more()) {
                        token = tok.nextContent();
                        if (token == LT) {
                            token = tok.nextToken();
                            if (token == SLASH) { // parent end
                                tok.nextToken();
                                break;
                            } else if (token instanceof String) {
                                String tag = (String) token;
                                if (tag.equals(childKey)) {
                                    childReplaced = true;
                                    if (trailingSlash && replacement.length() == 1) {
                                        String onlyKey = replacement.keys().next();
                                        parentObj.put(childKey, replacement.get(onlyKey));
                                    } else
                                        parentObj.put(childKey, replacement);

                                    // skip original child content entirely
                                    int depth = 1;
                                    while (depth > 0 && tok.more()) {
                                        token = tok.nextToken();
                                        if (token == LT) {
                                    ...
                                    tok.back();
                                    parse(tok, parentObj, null, cfg, 0);
                                }
                            }
                        }
                    }
                    doc.put(parentKey, parentObj);
                } else {
                    tok.back();
                    parse(tok, doc, null, cfg, 0);
                }
    ...
    /** Builds a pointer representing the parent of {@code pointer}. */
    private static JSONPointer parentPointer(JSONPointer pointer) {
        String raw = pointer.toString();
